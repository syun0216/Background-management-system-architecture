...
-
  ...
  ```
当用户登录后，我们在store和cookie里面分别存了一份用户的token，当我们退出登录的时候同样是要把我们的store和cookie等缓存清空

- 菜单管理<br>
  一般来说后台系统菜单是通过后台获取而来的，并且是可以通过用户角色的不同来得到相应的菜单的。但是路由我们必须在本地配一套，如果用户不多的情况下，路由是分实例路由（任何人都可见的），还有动态路由。通过我们登录之后在路由钩子之前使用addRoute方法动态加载进去，
